package Braint;
 ...
 public class BraintUtil {
	...
	static public double getNormalizedValueLog(double value, double max, double min, double maxScaled,
			double minScaled) {
		return Math.log10(minScaled) + (Math.log10(value) - Math.log10(min))
				* (Math.log10(maxScaled) - Math.log10(minScaled)) / (Math.log10(max) - Math.log10(min));
	}

	static public double getNormalizedValueWuzzel(double value, double max, double min, double maxScaled,
			double minScaled) {
		return Math.sqrt(minScaled) + (Math.sqrt(value) - Math.sqrt(min))
